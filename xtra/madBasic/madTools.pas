// ***************************************************************
//  madTools.pas              version: 1.3.3  ·  date: 2015-04-20
//  -------------------------------------------------------------
...
type
  ...
  TOsEnum = (osNone, osWin95, osWin95osr2, osWin98, osWin98se, osWinME, osWin9xNew,
                     osWinNtOld, osWinNt4, osWin2k, osWinXP, osWin2003, osWinVista, osWin2008, osWin7, osWin2008r2, osWin8, osWin2012, osWin81, osWin2012r2, osWin10, osWinNtNew);
  TOS = record
    major       : cardinal;
    minor       : cardinal;
    build       : cardinal;
  ...
  COsDescr : array [TOsEnum] of PAnsiChar =
             ('None', 'Windows 95', 'Windows 95 OSR-2', 'Windows 98', 'Windows 98 SE',
                      ...
                      'Windows NT 3', 'Windows NT 4', 'Windows 2000', 'Windows XP',
                         ...
                         'Windows 8', 'Windows 2012', 'Windows 8.1', 'Windows 2012 R2', 'Windows 10', 'Windows NT New');
...
begin
  ...
  if nh <> nil then begin
    if nh^.OptionalHeader.Magic = IMAGE_NT_OPTIONAL_HDR64_MAGIC then
         NativeUInt(result) := module + PImageOptionalHeader64(@nh^.OptionalHeader).DataDirectory[directory].VirtualAddress
    else NativeUInt(result) := module +                         nh^.OptionalHeader .DataDirectory[directory].VirtualAddress;
  end else
    result := nil;
end;
...
begin
  result := addr;
  if nh^.OptionalHeader.Magic = IMAGE_NT_OPTIONAL_HDR64_MAGIC then
       NativeUInt(sh) := NativeUInt(@nh^.OptionalHeader) + sizeOf(TImageOptionalHeader64)
  else NativeUInt(sh) := NativeUInt(@nh^.OptionalHeader) + sizeOf(TImageOptionalHeader32);
